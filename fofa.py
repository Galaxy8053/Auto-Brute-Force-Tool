...
# --- **关键修复：导入时区库和跨平台检测库** ---
# --- Key Fix: Import timezone and cross-platform detection libraries ---
...
def get_system_timezone_name():
    """
    使用 tzlocal 库以跨平台方式自动获取系统的时区名称。
    Uses the tzlocal library to automatically get the system's timezone name in a cross-platform way.
    """
    try:
        tz_name = tzlocal.get_localzone_name()
        # 验证时区名称是否有效 (pytz可以处理 tzlocal 可能返回的链接)
        # Validate the timezone name (pytz can handle links that tzlocal might return)
        timezone(tz_name)
        return tz_name
    except Exception as e:
        logger.warning(f"无法使用 tzlocal 自动检测时区: {e}。将默认使用 UTC。")
        logger.warning(f"Could not auto-detect timezone using tzlocal: {e}. Falling back to UTC.")
        return 'UTC'
...
def _make_request(url: str):
    ...
    try:
        ...
        res.raise_for_status(); data = res.json(); return data, data.get("errmsg")
...
async def stop_all_tasks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    job_name = f"download_job_{chat_id}"
    current_jobs = context.job_queue.get_jobs_by_name(job_name)
    if not current_jobs: await update.message.reply_text("目前没有正在运行的下载任务。"); return
    for job in current_jobs: job.schedule_removal()
    await update.message.reply_text("✅ 已强制停止所有后台下载任务。")

# --- Bot 命令 & 对话流程 (此处省略以保持简洁, 与上一版相同) ---
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.message.reply_text('👋 欢迎使用 Fofa 查询机器人！请使用 /help 查看命令手册。', parse_mode=ParseMode.MARKDOWN)
...
async def kkfofa_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if total_size <= 10000:
        await msg.edit_text(f"{success_message}\n开始下载..."); _start_download_job(context, run_full_download_query, context.user_data); return ConversationHandler.END
    else:
...
async def query_mode_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == 'mode_full': await query.edit_message_text(f"⏳ 开始全量下载任务..."); _start_download_job(context, run_full_download_query, context.user_data)
    elif mode == 'mode_traceback': await query.edit_message_text(f"⏳ 开始深度追溯下载任务..."); _start_download_job(context, run_traceback_download_query, context.user_data)
...
async def show_api_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if CONFIG['apis']:
            ...
            for i, (data, error) in enumerate(results):
                key_masked = f"`{CONFIG['apis'][i][:4]}...{CONFIG['apis'][i][-4:]}`"; status = f"❌ 无效"
                if not error:
        ...
        logger.error(f"显示 API 菜单时出错: {e}", exc_info=True); await context.bot.send_message(update.effective_chat.id, "显示菜单时发生内部错误，请重试。"); return ConversationHandler.END
...
async def settings_action_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == 'back_main': return await settings_command(update, context)
        ...
        if not CONFIG['apis']: await context.bot.send_message(update.effective_chat.id, "没有可删除的API Key。"); return await show_api_menu(update, context)
...
async def get_proxy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    class D: Q=type('Q',(),{'answer':(lambda:None),'edit_message_text':(lambda*a,**kw:update.message.reply_text(*a,**kw))}); callback_query=Q()
    return await show_proxy_menu(D(), context)
...
def main():
    ...
    # --- **终极修复：自动检测并应用时区** ---
    # --- Ultimate Fix: Automatically detect and apply timezone ---
    ...
    logger.info(f"成功检测到系统时区: {system_timezone_str}")
    logger.info(f"Successfully detected system timezone: {system_timezone_str}")
...
if __name__ == '__main__':
    # Fill in the omitted background tasks for completeness
    async def run_full_download_query(context: ContextTypes.DEFAULT_TYPE):
    ...
    async def run_traceback_download_query(context: ContextTypes.DEFAULT_TYPE):
        ...
        while True:
            page_count += 1; job_name = f"download_job_{chat_id}"
            if not context.job_queue.get_jobs_by_name(job_name): await msg.edit_text("🌀 深度追溯任务已手动停止。"); return
            ...
            if before_date == last_before_date: logger.warning("追溯日期未改变，为防止死循环，任务终止。"); break
