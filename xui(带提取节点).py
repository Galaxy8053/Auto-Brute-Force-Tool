...
PASSWORD_LIST = ['admin', '123456', 'test', 'abc123', 'user', '666666', '888888', '88888888', 'password', 'admin123',
                 '12345678', 'qwerty', '111111', '000000', 'root123', 'test123']
...
# 读取目标文件 (支持 .csv 和 .txt 的 ip:port 格式)
def read_targets(file_path):
    links = []
    encoding = detect_file_encoding(file_path)

    if file_path.lower().endswith('.csv'):
        print("检测到 CSV 文件，按 CSV 格式读取...")
        try:
            with open(file_path, 'r', newline='', encoding=encoding) as csvfile:
                reader = csv.DictReader(csvfile)
                for row in reader:
                    # 兼容 fofa 导出的 link 和 host 字段
                    link = row.get('link') or row.get('host')
                    if link:
    ...
    if not links:
        print(f"警告: 未能从 {file_path} 中读取到任何有效目标。")
...
def main(target_file, num_threads, use_proxies):
    links = read_targets(target_file)
...
if __name__ == "__main__":
    while True:
        filename_input = input("请输入包含目标的文件名 (例如 targets.txt 或 targets.csv): ").strip()
        if os.path.isfile(filename_input):
            xui_filename = filename_input
            break
        print(f"文件 '{filename_input}' 不存在，请重新输入")
